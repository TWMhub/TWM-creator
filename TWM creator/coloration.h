...
std::string getColorFromJson(std::string nameColor) {
	...
	std::string name = get—urrentPath() + "\\settings.json";
...
sf::Color SetColor(std::string col) { // ÔËÌËÏ‡ÂÚ Ì‡Á‚‡ÌËÂ ˆ‚ÂÚ‡ ‚˚‰‡∏Ú ˆ‚ÂÚ ËÁ sfml
...
sf::Color SetColor(std::string col, float alphaColor) { // ÔËÌËÏ‡ÂÚ Ì‡Á‚‡ÌËÂ ˆ‚ÂÚ‡ ‚˚‰‡∏Ú ˆ‚ÂÚ ËÁ sfml
	std::string color = getColorFromJson(col);
	if (color.length() != 7) {
		std::cerr << color.length();
		return sf::Color::Red;
